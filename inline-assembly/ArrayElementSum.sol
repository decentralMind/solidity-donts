...
contract arrayElementSum{
  ...
  /*
   * @dev Take note on memory management and free mmemory pointer. For the arguments [3,4], the free memory pointer
   ...
   * element 1 which is 4, free memory pointer 0x80 needs to increase by 64 bytes(0x40).
   **/
  ...
  function arraySum(uint[] memory _data) public pure returns(uint sum) {
    assembly {
      ...
      // Increase the length of data with 32 bytes.
